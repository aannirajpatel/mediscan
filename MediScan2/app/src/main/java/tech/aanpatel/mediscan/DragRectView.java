...
public class DragRectView extends View {
    ...
    protected void onDraw(final Canvas canvas) {
        super.onDraw(canvas);
        Log.i("INFO","PRINT: "+String.format("%d,%d,%d,%d",mStartX,mStartY,mEndX,mEndY)+String.format(",%d",Math.abs(mStartX-mEndX) * Math.abs(mStartY-mEndY)));
        if (mDrawRect) {
    ...
    public boolean getIsLegitRectangle(){
        if( Math.abs(mStartX-mEndX) > 0 && Math.abs(mStartY-mEndY) > 0 && mDrawRect){
            return true;
        }
        return false;
    }
